...
import { Add, Edit, Visibility, Search, Clear, SupervisorAccount } from '@mui/icons-material';
...
import UserFormModal from '../Members/UserFormModal.tsx';
import UserDetailModal from '../Members/UserDetailModal.tsx';
...
const StaffTable: React.FC = () => {
      ...
      const reloadData = async () => {
        try {
          const [usersRes, rolesRes] = await Promise.all([
            getUsersByRole('STAFF', 0, paginationState.rowsPerPage),
            getAllRolesLegacy()
          ]);
  ...
  const loadData = useCallback(async (page: number = 0, size: number = 10) => {
    ...
    try {
      ...
      const [usersRes, rolesRes] = await Promise.all([
        getUsersByRole('STAFF', page, size),
        getAllRolesLegacy()
      ]);
    ...
    return (
      <PageWrapper display="flex" justifyContent="center" alignItems="center" minHeight={360}>
        <Stack alignItems="center" spacing={2}>
          ...
          <Typography color="text.secondary" fontSize={14}>Loading staff...</Typography>
        </Stack>
      </PageWrapper>
    );
  }
          ...
          <Table sx={{ minWidth: { xs: 650, md: 750 } }}>
            <TableHead>
              <TableRow sx={{ backgroundColor: '#f8faff' }}>
                <TableCell sx={{ fontSize: { xs: '0.875rem', md: '1rem' }, fontWeight: 600, color: '#0f172a' }}>Staff</TableCell>
            ...
            <TableBody>
              {filteredStaff.map((staffMember) => (
                <TableRow key={staffMember.id} hover sx={{ '&:hover': { backgroundColor: '#f8faff' } }}>
                  <TableCell>
                    <Box display="flex" alignItems="center" gap={2}>
                      <Avatar src={staffMember.avatar} sx={{ width: { xs: 32, md: 40 }, height: { xs: 32, md: 40 } }}>
                  ...
                  <TableCell>
                    <Box display="flex" gap={0.5}>
                      ...
                      <LockButton
                          isLocked={staffMember.isActive === false}
                          onToggle={() => handleToggleStatus(staffMember.id)}
                          size="small"
                          lockedTooltip="Activate staff"
                          unlockedTooltip="Deactivate staff"
                      />
                    </Box>
                  </TableCell>
                </TableRow>
        ...
        <Box mt={3}>
          <TablePagination
            ...
            labelDisplayedRows={({ from, to, count }) =>
              `${from}–${to} of ${count !== -1 ? count : `more than ${to}`} staff`
            }
          />
        </Box>
