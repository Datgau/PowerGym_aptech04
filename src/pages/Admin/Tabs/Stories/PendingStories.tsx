...
const PendingStories: React.FC = () => {
  ...
  useEffect(() => {
    fetchPendingStories(paginationState.page, paginationState.rowsPerPage);
  }, [paginationState.page, paginationState.rowsPerPage]);

  const fetchPendingStories = async (page: number = 0, size: number = 12) => {
    setLoading(true);
    setError(null);
    try {
      const response = await storyService.getPendingStories(page, size);
    ...
    <Box>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Typography variant="h5" fontWeight={600}>
          Pending Stories ({paginationState.totalElements})
        </Typography>
        <Button
          variant="outlined"
          onClick={() => fetchPendingStories(paginationState.page, paginationState.rowsPerPage)}
          disabled={loading}
        >
          Refresh
        </Button>
