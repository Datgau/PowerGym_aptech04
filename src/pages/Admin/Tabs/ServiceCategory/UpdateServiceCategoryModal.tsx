...
const UpdateServiceCategoryModal: React.FC<UpdateServiceCategoryModalProps> = ({
  ...
  const formik = useFormik<UpdateServiceCategoryRequest>({
    initialValues: {
      displayName: '',
      description: '',
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        const response = await updateServiceCategory(category.id, {
          ...values,
          icon: selectedIcon,
          color: selectedColor,
        });
  ...
  useEffect(() => {
    if (category && open) {
      ...
      setSelectedIcon(category.icon || DEFAULT_ICONS[0]);
    }
          ...
          <Box container spacing={3}>
            ...
            <Grid item xs={12} sx={{ mt: 2, mb: 2 }}>
              <StyledTextField
                ...
                value={category.name}
                disabled
                helperText="System identifier cannot be changed"
              />
            </Grid>
