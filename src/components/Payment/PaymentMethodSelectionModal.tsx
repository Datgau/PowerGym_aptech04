        ...
        >
          ...
          {serviceName && (
              ...
              >
                ...
                <Typography sx={{ fontFamily: '"Sora", sans-serif', fontSize: '0.85rem', color: 'rgba(255,255,255,0.9)', fontWeight: 500 }}>
                  {serviceName}
                </Typography>
                {amount && (
                    <>
                      <Box sx={{ width: 1, height: 14, backgroundColor: 'rgba(255,255,255,0.25)' }} />
                      <Typography sx={{ fontFamily: '"Sora", sans-serif', fontSize: '0.85rem', color: 'white', fontWeight: 700 }}>
                        {formatAmount(amount)}
                      </Typography>
                    </>
                )}
        ...
        <DialogContent sx={{ px: 3, pt: 0, pb: 3 }}>
          <Stack spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Box
                ...
                onMouseEnter={() => setHoveredCard('momo')}
                onMouseLeave={() => setHoveredCard(null)}
                sx={{
                  cursor: 'pointer',
                  borderRadius: '16px',
                  background: hoveredCard === 'momo'
                      ? 'linear-gradient(135deg, #a0026a 0%, #d82d8b 100%)'
                      : 'white',
                  border: hoveredCard === 'momo' ? '2px solid transparent' : '2px solid #ebebf0',
                  transition: 'all 0.28s cubic-bezier(0.34, 1.56, 0.64, 1)',
                  transform: hoveredCard === 'momo' ? 'translateY(-3px) scale(1.01)' : 'translateY(0) scale(1)',
                  boxShadow: hoveredCard === 'momo'
                      ? '0 16px 40px rgba(216,45,139,0.35)'
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                <Box
                    sx={{
                      ...
                      border: hoveredCard === 'momo' ? '1px solid rgba(255,255,255,0.3)' : 'none',
                    }}
                >
                  <PhoneAndroid sx={{ color: 'white', fontSize: 26 }} />
                </Box>
                ...
                <Box sx={{ flex: 1, minWidth: 0 }}>
                  <Typography
                      sx={{
                        ...
                        fontWeight: 700,
                        fontSize: '1rem',
                        color: hoveredCard === 'momo' ? 'white' : '#1a1a2e',
                  ...
                  <Typography
                      sx={{
                        fontFamily: '"Sora", sans-serif',
                        fontSize: '0.78rem',
                        color: hoveredCard === 'momo' ? 'rgba(255,255,255,0.75)' : '#7a7a8c',
                        transition: 'color 0.2s',
                      }}
                  >
                    Scan QR code — instant confirmation
                  </Typography>
                  <Box sx={{ display: 'flex', gap: 0.8, mt: 1 }}>
                    {['Instant', 'Secure'].map((tag) => (
                        <Typography
                            key={tag}
                            sx={{
                              fontSize: '0.68rem',
                              ...
                              px: 1, py: 0.3,
                              borderRadius: '6px',
                              backgroundColor: hoveredCard === 'momo' ? 'rgba(255,255,255,0.18)' : (tag === 'Instant' ? '#e8f5e8' : '#e3f2fd'),
                              color: hoveredCard === 'momo' ? 'rgba(255,255,255,0.9)' : (tag === 'Instant' ? '#2e7d32' : '#1565c0'),
                              transition: 'all 0.2s',
                ...
                <Box
                    sx={{
                      width: 34, height: 34,
                      borderRadius: '10px',
                      backgroundColor: hoveredCard === 'momo' ? 'rgba(255,255,255,0.2)' : '#fce4f4',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      transition: 'all 0.28s ease',
                      flexShrink: 0,
                    }}
                >
                  <ArrowForward sx={{ fontSize: 18, color: hoveredCard === 'momo' ? 'white' : '#d82d8b', transition: 'color 0.2s' }} />
                </Box>
              </Box>
            </Box>
            ...
            >
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    ...
                    {['Manual Verification', 'Secure'].map((tag) => (
                        <Typography
                            ...
                            sx={{
                              ...
                              px: 1, py: 0.3,
                              borderRadius: '6px',
                              backgroundColor: hoveredCard === 'bank' ? 'rgba(255,255,255,0.18)' : (tag === 'Manual Verification' ? '#fff3e0' : '#e3f2fd'),
                              color: hoveredCard === 'bank' ? 'rgba(255,255,255,0.9)' : (tag === 'Manual Verification' ? '#e65100' : '#1565c0'),
                              transition: 'all 0.2s',
                              letterSpacing: '0.03em'
                            }}
                        >
                ...
                >
                  <ArrowForward sx={{ fontSize: 18, color: hoveredCard === 'bank' ? 'white' : '#1366ba', transition: 'color 0.2s' }} />
                </Box>
              </Box>
            </Box>
          </Stack>
